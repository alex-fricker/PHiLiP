...
::select_test(const AllParam *const parameters_input,
    ...
    //   if constexpr (dim==2 && nstate==dim+2) return std::make_unique<EulerGaussianBumpAdjoint<dim,nstate>>(parameters_input);
        ...
        if constexpr ((dim==2 && nstate==dim+2) || (dim==1 && nstate==1 || dim==3 && nstate==dim+2)) return std::make_unique<AdaptiveSampling<dim,nstate>>(parameters_input,parameter_handler_input);
