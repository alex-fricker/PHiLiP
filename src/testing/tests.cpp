...
::select_test(const AllParam *const parameters_input,
    ...
    } else if(test_type == Test_enum::neural_network_rom) {
        if constexpr ((dim==2 && nstate==dim+2) || (dim==1 && nstate==1))  return std::make_unique<NeuralNetworkROM<dim, nstate>>(parameters_input, parameter_handler_input);
    } else {
